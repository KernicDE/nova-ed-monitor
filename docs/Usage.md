...
- **Materials** — full grade-based material tracker for Raw (G1‑G4), Manufactured (G1‑G5), and Encoded (G1‑G5). Every material is shown as one compact row: `Category | Grade | Name | Count/Cap | [Bar] | Pct%`. Columns are aligned across all three material types so every table lines up vertically. The progress bar expands dynamically to fill whatever panel width remains after the other columns are fixed. Names truncate with `…` on narrow terminals; full names appear on wide terminals. Colours indicate stock level: dim grey for empty, white for partial, amber for near-cap, and green for well-stocked (≥80%).
