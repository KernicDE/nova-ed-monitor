...
.SYNOPSIS
    ...
    Portable launcher for Windows — all data lives next to this script.

.DESCRIPTION
    Auto-updates itself and NOVA from GitHub, then launches NOVA.
    Installs Python and NOVA automatically if not present.
    All data (config, database, venv) stays in the same directory as this script.

.PARAMETER NoSelfUpdate
    Skip the self-update check (used internally after auto-update).

.PARAMETER Uninstall
    Completely remove the NOVA directory. Does not touch Elite Dangerous journal files.
...
$PortableRoot = Split-Path $ScriptPath -Parent
...
if (-not (Test-Path $VenvDir)) {
    ...
    @("config","data","logs") | ForEach-Object {
        New-Item -ItemType Directory -Path (Join-Path $PortableRoot $_) -Force | Out-Null
    }
    ...
    if ($latestVer -and $installedVer -ne $latestVer) {
        ...
        if ($whlUrl) {
            ...
            & $VenvPython -m pip install --quiet --upgrade $NOVA_URL
        }
